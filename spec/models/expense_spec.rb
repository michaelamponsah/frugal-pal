...
RSpec.describe Expense, type: :model do
  describe 'field validations' do
    it 'is invalid without a name' do
      ...
      expense = FactoryBot.build(:expense, name: nil, user:)
      expect(expense).not_to be_valid
    end

    it 'is invalid without an amount' do
      user = FactoryBot.build(:user)
      expense = FactoryBot.build(:expense, amount: nil, user:)
      expect(expense).not_to be_valid
    end

    it 'is valid with a name, amount and a user' do
      user = FactoryBot.build(:user)
      expense = FactoryBot.build(:expense, user:)
  ...
  describe 'associations' do
    it 'should have an associated user' do
      ...
      expense = FactoryBot.create(:expense, user:)
